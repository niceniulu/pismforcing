...
def downscale_ECHAM2PISMgrids(fecham='select_ECHAM6.nc', fpismgrid='pismr_nhem_20km.griddes', fpismcurrent='20km_lrno_pismr_out_-021100--021001.nc',
    interpmethod='bil' , 
    ...
    #------------------------------------------------
    print("..... regrid ECHAM6 to PISM grids using cdo:", interpmethod, "| output: ", fout )
    ...
    #------------------------------
    print("..... Downscaling temperature with option: ", downscaleT, ", and number",factorT, "K/km")
    if  downscaleT == 'linear' :
        if factorT > 0: 
            sys.exit('!!!!! DownscaltT: '+str(factorT) +' should be negative (K/km), Stop')
        elif  factorT < 0 and factorT > -1 :
            sys.exit('!!!!! DownscaltT: '+str(factorT) +' should be with unit K/km, Stop')
        #
    ...
    # elif : to be implemented 
        ...
        print("..... no downscaling Temperature applied. ")
        T_downscaled = temp2
    ...
    #------------------------------
    print("..... Downscaling precipitation with option: ", downscaleP, ", need factor:",factorP)
    ...
    # elif : to be implemented 
        ...
        print("..... no downscaling Precipitation applied. ")
        P_downscaled = precip 

    #------------------------------
    # Write to output 
    #------------------------------
    print("..... Write to output file: ", fout ," | new vars: air_temp, precipitation ")
...
def select_vars_for_PISM(fin='dummy_ECHAM6.nc', schemeSMB='PDD',fout='select_ECHAM6.nc'):
    #
    g = 9.81  # m/s^2
    sigma = 5.6703744e-8
    ...
    with nc.Dataset(fin,'r') as src, nc.Dataset(fout,'w') as dst:
        ...
        print("..... Select and modify vars for SMB scheme: ", schemeSMB)
        ...
        if schemeSMB == 'PDD':
            ...
            print("..... unknown SMB scheme", schemeSMB, ", generate orog, temp2, aprt only")
    #
    return 


def run_dEBM(fin='downscaled.nc',obliquity=None, exedEBM=None):
    ff = nc.Dataset(fin,'r')
    ...
    for i in vars:
        try:
            ...
            sys.exit('!!!!!  Variable: '+ i +"  is missing. Stop !!!!!" )
        
    if obliquity is None: 
        sys.exit("!!!!! Need to specify the obliquity value for dEBM, Stop. ")
    #
    if exedEBM is None:
        sys.exit("!!!!! Need to specify the path for exe dEBM, Stop. ")
    #
    # write to namelist.debm 
    print("..... generate namelist.debm ")
    ...
    # run dEBM 
    print("..... running dEBM, ./dEBMmain  namelist.debm ")
    os.system("cp   "+exedEBM+"    ./dEBMmain")
    os.system("./dEBMmain  namelist.debm  ")

    print("..... Done. | output: surface_mass_balance.nc ")
    return 



def generate_atmos_forcing_for_PISM(fin='downscaled.nc',fout='PISM.nc',scheme='PDD',fileplus1=None,timefreq = "12month"):
    print("..... generate atmos. forcing for PISM with scheme: ", scheme)

    if scheme == 'PDD':
        print('..... pdd')
        os.system(" ncks  -O -v  air_temp,precipitation    "+fin+"  "+ fout )
        ...
        if fileplus1 is None:
            sys.exit("!!!!! Need to specify outputfile from dEBM (fileplus1), Stop. ")
    ...
    print("..... add time and time bounds dimensions")
    with nc.Dataset(fout,'r+' ) as ff:
        ...
        if timefreq == "12month":
            ...
            sys.exit("..... not 1-year monthly data, stop.")
